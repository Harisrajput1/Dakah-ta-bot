...
class AFK(commands.Cog):
    ...
    async def afk(self, ctx, *, reason="busy so don't ping"):
        ...
        await ctx.send(f"<a:Diamonds:1272969331596198010> **Successfully Set AFK**")
        self.save_afk_data()
    ...
    async def unafk(self, ctx):
        ...
        if user_id in self.afk_data:
            ...
            await ctx.send(f"<a:Diamonds:1272969331596198010> **Successfully Removed AFK**")
            self.save_afk_data()
        else:
        ...
        for user_id, reason in self.afk_data.items():
            if f"<@{user_id}>" in message.content:
                if message.author.id not in self.user_cooldowns:
                    await message.channel.send(f"<a:Diamonds:1272969331596198010> {message.author.mention}, I am afk rn., **{reason}**")
